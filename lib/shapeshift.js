...
// Globally store API tokens
const tokens = {}

exports.setTokens = function (publicKey, privateKey) {
  Object.assign(tokens, { publicKey, privateKey })
}
...
function _shift (withdrawalAddress, pair, options, callback) {
  ...
  payload.pair = pair
  if (tokens.publicKey) {
    payload.apiKey = tokens.publicKey
  }
...
exports.transactions = function (address, callback) {
  const apiKey = tokens.privateKey
  if (!apiKey) {
    throw new Error('Missing private key. Use: shapeshift.setTokens()')
  }
